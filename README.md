...
3. Confirm the sensor is visible: `i2cdetect -y 1`. You should see a device at `0x76`, or at `0x77` depending on the breakout's address pin.
...
1. Build the self-extracting installer:
   ...
   This produces `bme280mon-v0.1.0-linux-arm64.install`. It is a single executable shell script with the `bme280mon` binary, the systemd units, and the example configs bundled inside it.
...
2. Copy it to the Pi and run it as root:
   ...
   - installs the example config to `/etc/bme280mon/config.yaml`, but only if a config doesn't already exist there. Re-running the installer won't clobber your settings.
...
3. **Edit the config** before starting the service:
   ...
   At minimum, set `ntfy_topic` to a unique, hard-to-guess topic name. It is effectively a secret, as described below. Adjust `humidity_threshold`, `humidity_buffer`, `poll_interval`, and the rest if the defaults don't suit your room or environment.
...
[ntfy.sh](https://ntfy.sh) is a free push-notification service: you publish to a "topic" (just a URL path) and anyone subscribed to that topic name gets a push notification. There is no account or registration. The topic name itself is the access control, so **treat it like a secret**: pick something long and hard to guess, such as a random string, not something like `my-humidity-alerts`.
...
`bme280mon` serves Prometheus-format metrics over HTTP. The shipped example config sets `metrics_addr: "127.0.0.1:9101"`, so the endpoint is reachable only from the Pi itself. That is all the local scraper needs. Read it on the Pi, or through an SSH tunnel from elsewhere:
...
Point a Prometheus or VictoriaMetrics scrape config at `http://127.0.0.1:9101/metrics` to collect these over time. See [the monitoring stack guide](docs/monitoring-stack.md) for a scraper, database, and Grafana that install onto the same Pi. Scraping from another machine means changing `metrics_addr` to a network-reachable address (for example `:9101`) and restarting. Be aware that this exposes the endpoint, unauthenticated, to your whole LAN.
...
To store and graph these readings, this repo ships a companion installer that sets up VictoriaMetrics and Grafana on the same Pi. See [docs/monitoring-stack.md](docs/monitoring-stack.md).
...
Note that the Pi has fixed 1.8k ohm pull-ups on GPIO2/GPIO3 and each breakout adds its own, so the pull-ups get stronger as you add boards. Two breakouts stay within spec. Keep the runs short.
...
- `metrics_addr`: the two instances cannot bind the same port. A clash is a startup failure, announced over ntfy, rather than a silently metric-less daemon.
